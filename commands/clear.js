...
module.exports = {
    ...
    async execute(message, args, admin_role) {
        ...
        var isAdmin = await utils.hasRole(message.author, message.guild, admin_role);
        ...
        console.log(`Intentando borrar ${amount-1} mensajes de ${message.channel.name}`);


        if (amount - 1 >= 100) {
            await message.author.send("Lo siento, no puedes borrar mas de 99 mensajes"
